...
foreach($output as $k => $f)
{
  if(strlen($k)>3)
  {
    ...
    if(strlen($bin)>0)
    {
      ...
      "genre"=>$genre,"url"=>$url,"sourceUrl"=>$sourceUrl,"author"=>$author,"banner"=>$banner,"date"=>$modified,"binaries"=>$hex,"screenshots"=>$screenshots,"license"=>$license,"id"=>substr(md5($author . '_' . $title),0,6), "arduboy"=>"");
    }
  }
}
